...
def build_sample_amplitudes(num_qubits: int) -> np.ndarray:
    """Build the sample input state used across demos and benchmarks."""
...
def build_measured_qft_circuit(amplitudes: np.ndarray, qft_circuit: QuantumCircuit) -> QuantumCircuit:
    """Attach measurements so a QFT circuit can be sampled."""
...
def require_aer():
    """Load Aer only when it is requested."""
    try:
...
def sample_aer_counts(
    ...
    *,
    ...
    simulator = (
        AerSimulator(**simulator_kwargs)
        if method == "automatic"
        else AerSimulator(method=method, **simulator_kwargs)
    )
